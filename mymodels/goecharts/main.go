...
import (
	"math/rand"
	"time"
	"godev/mymodels/goecharts/utils"
...
func handler(w http.ResponseWriter, _ *http.Request) {
	...
	bar.SetGlobalOptions(charts.TitleOpts{Title: "Bar-标记线&标记点"},
		charts.InitOpts{PageTitle:"this is tj first go-charts"})
	...
	bar.Render(w) // Render 可接收多个 io.writer 接口
}
...
func handler3D() *charts.Bar3D{
	...
	bar3d.AddXYAxis(utils.Hours, utils.Days).AddZAxis("bar3d", genBar3dData())

	//bar3d.Render(w) // Render 可接收多个 io.writer 接口
	return bar3d
}

func pages(w http.ResponseWriter, _ *http.Request) {
	page := charts.NewPage(utils.OrderRouters("bar")...)
	page.InitOpts=charts.InitOpts{PageTitle:"this is tj first go-charts"}
	page.Add(
		handler3D(),
		bar(),
...
func bar() *charts.Bar{
	bar := charts.NewBar()
	bar.SetGlobalOptions(charts.TitleOpts{Title: "Bar-标记线&标记点"},charts.InitOpts{PageTitle:"this is tj first go-charts"})
	bar.AddXAxis(nameItems).
		AddYAxis("商家A", randInt()).
		AddYAxis("商家B", randInt())
	bar.SetSeriesOptions(
...
func main() {
	...
	http.ListenAndServe(strings.Join([]string{":",utils.Port},""), nil)
}