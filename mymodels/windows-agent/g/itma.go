...
func GetHardware() (string, string, string, string,error) {
	if hardware {
		return manufacturer, productName, version, serialNumber,nil
	}
	cmd := exec.Command("cmd", "/c", "wmic BaseBoard get Manufacturer,Product,Version,SerialNumber")
	stdout, err := cmd.StdoutPipe()
   if err != nil {
	   return manufacturer, productName, version, serialNumber,err
	}
	cmd.Start()
	...
	for {
		...
		if strings.Contains(string(line), "Product") {
			continue
		}
		_data_sile := strings.Fields(string(line))
		if len(_data_sile) > 0 {
			manufacturer = _data_sile[0]
			productName = _data_sile[1]
			serialNumber = _data_sile[2]
			version = _data_sile[3]
		}else{
			continue
		}
	}
...
func EnvGrid() model.EnvGrid {
    ...
    manufacturer, productName, version, serialNumber ,err:= GetHardware()
	if err != nil{
		fmt.Println(err)
	}
